//solo life
//in this program, we write 8 functions. 
//A slicing floorplan can be obtained by repetitively cutting the floorplan horizontally or vertically, 
// whereas a non-slicing floorplan cannot.  
// The floorplan here is a slicing floorplan since each module can be sliced out through a sequence 
// of vertical cut or horizontal cut. 
// On the other hand, we cannot apply this cutting process to slice out any modules from the floorplan.
...
int is_in_subtree(node_t* a, node_t* b) {
  ...
  while (c) //check if b is root
  {
    if (a==c->parent)
    {
      return 1;
    }
    c = c->parent; //loop up till reach a
  }
  return 0;
}
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // TODO:
  node_t* c = a->parent;  //copy the parent
  if (a->parent->right == a)  //check both side
  {
    a->parent->right = b;
  }
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  if (ptr->left==NULL && ptr->right==NULL)
  {
    expression[*nth].module = ptr->module;  //copy
    (*nth)++; //increase index
  }
  else
  {
    expression[*nth].cutline = ptr->cutline;  //copy
    (*nth)++;
  }
...
int get_total_resource(node_t* ptr)
{
  ...
  {
    sum+=modules[i].resource; //loop to sum all the resorce
  }
  return sum;
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  node_t* left_ = (node_t*) malloc(sizeof(node_t)); //create the left node
  left_->parent = par;
  left_->cutline = V;
  left_->module = NULL;
  left_->left = NULL;
  ...
  if (par)  //check if it is the begining. if par is NULL, should not create the right node. 
  {
    par->right = (node_t*) malloc(sizeof(node_t));
    par->right->module = &modules[n-1];
    par->right->cutline = UNDEFINED_CUTLINE;
  ...
  init_slicing_tree(left_, n+1);  //recursive
